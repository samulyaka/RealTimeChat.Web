function initThirdPartLibs() {
    initScreenHeight();
    initFlexibleTextArea();
    initCustomForms();
    initDatepicker();
    initLightbox();
    initPopups();
...
function initAccordion1() {
	jQuery('ul.footer-accordion').slideAccordion({
		opener: 'a.opener',
		slider: 'div.slide',
		animSpeed: 300
	});
...
function destroyAccordion(){
	jQuery('ul.footer-accordion').trigger('destroyAccordion');
}

// accordion menu init
function initAccordion() {
	jQuery('.accordion-panel').slideAccordion({
		opener: 'a.opener',
		slider: 'div.slide',
		animSpeed: 300,
		onAnim: function(state) {
			jcf.refresh('.content-box');
			if (state) {
				setTimeout(function() {
...
function initScreenHeight() {
	jQuery('.main > .jcf-scrollable').screenHeight({
		skipBoxes: '#header, .message-form',
		currProp: 'height',
		onResize: function(type) {
			if (type === 'refresh') {
				jcf.refresh(this.box);
			}
		}
	});
	jQuery('.inner .main > .jcf-scrollable').screenHeight({
		skipBoxes: '#header',
		currProp: 'height',
		onResize: function(type) {
			if (type === 'refresh') {
...
// flexible textarea inint
function initFlexibleTextArea() {
	jQuery('textarea.flexible').each(function() {
		var textarea = jQuery(this);
		var currHaight = textarea.height();
		var prevHeight = currHaight;

		textarea.flexText();
		textarea.on('keyup', function() {
			currHaight = textarea.height();

			if (currHaight !== prevHeight) {
				jQuery(window).trigger('refresh');
				prevHeight = currHaight;
			}
		});
	});
}
...
function initOpenClose() {
	jQuery('.open-close').openClose({
		...
		animSpeed: 400,
		effect: 'slide',
		animEnd: refreshScrollAreas
	});
	/*jQuery('.open-close-section').openClose({
		activeClass: 'active',
	...
	function refreshScrollAreas() {
		jQuery('.jcf-scrollable').each(function() {
...
function initPopups() {
	jQuery('.drop-holder').contentPopup({
		...
		onInit: function() {
			var self = this;
			this.scrollBox = this.holder.closest('.jcf-scrollable');
			...
			this.popup.on({
				mouseenter: function() {
					self.holder.closest('.action-container').addClass('hover');
				},
				mouseleave: function() {
					self.holder.closest('.action-container').removeClass('hover');
				}
			});
		},
		onOutside: function() {
			this.scrollBox.on('scroll', this.hideHandler);
		},
		onInside: function() {
			this.scrollBox.off('scroll', this.hideHandler);
		},
	...
	jQuery('.popup-holder-1').contentPopup({
		mode: 'click',
...
function initMobileNav() {
	jQuery('body').mobileNav({
		...
		menuActiveClass: 'mobile-menu',
		menuOpener: '.mobile-button, .nav-opener',
		menuDrop: '.left-panel',
		onBeforeShow: refreshScrollAreas,
		onBeforeHide: refreshScrollAreas
	});

	jQuery('body').mobileNav({
		menuActiveClass: 'panel-showing',
		menuOpener: '.open-bar',
		onBeforeShow: refreshScrollAreas,
		onBeforeHide: refreshScrollAreas
	});
	...
	jQuery('#wrapper').mobileNav({
		...
		menuOpener: '.search-modal-open'
	});

	jQuery('.day-msgs').mobileNav({
		hideOnClickOutside: true,
		menuActiveClass: 'action-active',
		menuOpener: '.drop-open',
		menuDrop: '.day-msgs'
	});

	jQuery('.num-list>li').mobileNav({
		hideOnClickOutside: true,
		menuActiveClass: 'action-active',
		menuOpener: '.drop-open',
		menuDrop: '.num-list>li'
	});
...
/*
 * jQuery screen height plugin
 */
;(function($) {
	function ScreenHeight(options) {
		this.options = $.extend({
			skipBoxes: '#header',
			currProp: 'min-height'
		}, options);
	...
	ScreenHeight.prototype = {
		...
		findElements: function() {
			...
			this.skipBoxes = $(this.options.skipBoxes);
		},
		attachEvents: function() {
			...
			this.resizeHandler = function(e) {
				self.setBoxHeight();
				self.makeCallback('onResize', e.type);
			};

			this.resizeHandler({});
			win.on('resize orientationchange load refresh', this.resizeHandler);
		},
		getSkipBoxesHeight: function() {
			var currHeight = 0;

			this.skipBoxes.each(function() {
		...
		setBoxHeight: function() {
			this.box.css(this.options.currProp, (this.getWindowHeight() - this.getSkipBoxesHeight()) + 'px');
		},
		...
		destroy: function() {
			...
			win.off('resize orientationchange load refreshBoxScreenHeight', this.resizeHandler);
			setTimeout(function() {
				self.box.css(self.options.currProp, '');
			}, 1);
		}
	};
	...
	$.fn.screenHeight = function(opt) {
		return this.each(function() {
			var box = jQuery(this);
			var instance;

			if (typeof opt === 'object' || typeof opt === 'undefined') {
				instance = new ScreenHeight($.extend({
					box: this
				}, opt));
				box.data('ScreenHeight', instance);
			} else if (typeof opt === 'string' && box.data('ScreenHeight')) {
				instance = box.data('ScreenHeight');
				instance[opt]();
			}
		});
	};
...
;(function($) {
	...
	ContentPopup.prototype = {
		...
		goOutside: function() {
			...
			this.popup.appendTo($('body')).css(popupoffset);
			$(window).on('resize orientationchange', this.resizeHandler);
			this.makeCallback('onOutside');
		},
		goInside: function() {
			this.popup.appendTo(this.holder).css({
				left: '',
				top: ''
			});
			$(window).off('resize orientationchange', this.resizeHandler);
			this.makeCallback('onInside');
		},
		makeCallback: function(name) {
			if(typeof this.options[name] === 'function') {
				var args = Array.prototype.slice.call(arguments);
	...
	$.fn.slideAccordion = function(opt){
		...
		return this.each(function(){
			...
			items.each(function(){
						...
						if(item.hasClass(options.activeClass)) {
							if(options.allowClickWhenExpanded) {
								...
								slider.slideUp(options.animSpeed, function(){
									...
									if(jQuery.isFunction(options.onAnim)){
										options.onAnim(false);
									}
								});
							}
						} else {
				...
				accordion.on('destroyAccordion',function(){
					showSlide(slider);
